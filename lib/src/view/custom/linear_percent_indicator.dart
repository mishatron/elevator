import 'package:elevator/src/view/custom/bottom_menu/paint/half_painter.dart';
...
class LinearPercentIndicator extends StatefulWidget {
  ...
  LinearPercentIndicator(
    ...
    if (currentPoints == null ||
        currentPoints > pointsMaximum ||
        currentPoints < 0) {
      throw new Exception(
          "currentPoints value must be a integeer between 0 and $pointsMaximum");
    }
  }

  double get percent => currentPoints / pointsMaximum;
  ...
  Widget build(BuildContext context) {
    ...
    var containerWidget = Container(
      ...
      child: CustomPaint(
          ...
          child: Center(
              child: Text(
                  "${widget.currentPoints} / ${widget.pointsMaximum}"))),
    );
...
class LevelPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    drawLevelSymbol(canvas, size);
  }

  void drawLevelSymbol(Canvas canvas, Size size) {
    _paintCircle.color = backgroundColor;
    _paintCircle.style = PaintingStyle.stroke;
    _paintCircle.strokeWidth = 2;
    final padding = 4.0;
    double radius = (size.height - padding - padding) / 2;
    Offset center = Offset(0, padding + radius);
    canvas.drawCircle(center, radius, _paintCircle);

//    Rect rect = Rect.fromLTWH(0, 0, size.width/2, size.height);
//
//    Paint p1 = Paint()..color = progressColor;
//    Paint p2 = Paint()..color=progressColor;
//    p2.blendMode = BlendMode.xor;
//
//    canvas.drawRect(rect, p1); // Draws a red rect.
//    canvas.saveLayer(null, p2); // Saves the whole canvas.
//
//    canvas.drawRect(
...
//    canvas.restore();
    ...
    TextSpan span = TextSpan(
        style: TextStyle(color: backgroundColor), text: level.toString());
